...
interface Business {
  ...
  tax_number?: string;
  logo_url?: string;
  paystack_subaccount_code?: string; // present = verified with Paystack
...
// Shows a verified badge if the invoice's business has a Paystack subaccount,
// otherwise shows a CTA to set one up in Settings → Payment Collection.
function PaystackIntegrationCard({
  business,
  navigate,
}: {
  ...
  const hasSubaccount = !!business?.paystack_subaccount_code;
  const isVerified = !!business?.is_verified && hasSubaccount;
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        {isVerified ? (
          ...
          <span className="text-xs px-2 py-1 bg-amber-50 border border-amber-200 text-amber-700 rounded-full font-medium">
            Awaiting approval
          </span>
        ) : (
          <span className="text-xs px-2 py-1 bg-amber-50 border border-amber-200 text-amber-700 rounded-full font-medium">
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="w-9 h-9 rounded-lg bg-[#00C3F7]/10 flex items-center justify-center flex-shrink-0">
          <img
            src="/paystack_icon.svg"
            alt="Paystack"
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 truncate">
            {isVerified
              ? `Subaccount: ${business!.paystack_subaccount_code!.slice(0, 20)}…`
              : hasSubaccount
                ? "Pending Paystack approval"
      ...
      {isVerified ? (
        <div className="bg-green-50 border border-green-100 rounded-lg px-3 py-2.5 text-xs text-green-800 font-medium">
          Clients can pay this invoice online via Paystack.
        </div>
      ) : hasSubaccount ? (
        <div className="bg-amber-50 border border-amber-100 rounded-lg px-3 py-2.5 text-xs text-amber-800 font-medium">
          Your account is pending Paystack approval. Usually completed within 24
          hours.
        </div>
      ) : (
        <div className="space-y-3">
          <button
            onClick={() => {
              if (business?.name) {
                sessionStorage.setItem("paystack_connect", "true");
                sessionStorage.setItem("paystack_business", business.name);
              }
...
const InvoicePage = () => {
  ...
  // Re-fetch silently when tab regains focus
  useEffect(() => {
    const onFocus = () => fetchInvoice(true);
