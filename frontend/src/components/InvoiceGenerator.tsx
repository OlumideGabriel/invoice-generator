...
const InvoiceGenerator: React.FC = () => {
  ...
  const populateFormWithEditData = (editData: any) => {
    ...
    console.log(
      "✅ Form populated successfully. Invoice number:",
  ...
  const fetchInvoices = async () => {
    ...
    try {
      ...
      if (isEditing || selectedInvoiceId) {
        console.log(
          "✏️ fetchInvoices: edit mode - preserving existing invoice number:",
          invoiceNumber,
        );
        return;
      }
      ...
      if (!invoiceNumber) {
        ...
        console.log(
          "ℹ️ fetchInvoices: invoiceNumber already set, not generating:",
          invoiceNumber,
        );
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (fetchingEditData) {
      console.log(
        "⏸️ useEffect(fetchInvoices): fetchingEditData active, skipping",
      );
      return;
    }
    ...
    try {
      ...
      if (selectedInvoiceId) {
        ...
        response = await fetch(
          `${API_BASE_URL}/api/invoices/${selectedInvoiceId}`,
          {
            method: "PUT",
  ...
  return (
    <>
      ...
      <div className="md:hidden h-96 -z-10 -mb-96 top-5 bg-gradient-to-b from-teal-100 to-teal-0"></div>
