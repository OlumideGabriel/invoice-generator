...
const Dashboard: React.FC = () => {
  // Auth hook - replace with your actual auth hook
  const { user, loading: authLoading } = useAuth(); // Replace with your auth hook
  const userId = user?.id; // Adjust property name as needed

  // Early return for auth loading or unauthenticated users
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        // Updated to use user-specific endpoint
        const response = await fetch(`http://localhost:5000/api/dashboard?user_id=${userId}`);
  ...
  // Helper for user avatar initials (matching your existing pattern)
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="text-sm text-gray-500">
                  {dashboardData.stats.total_invoices > 0
                    ? Math.round((dashboardData.stats.paid_invoices / dashboardData.stats.total_invoices) * 100)
                    : 0}% of total
                </p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="text-sm text-gray-500">
                  {dashboardData.stats.total_invoices > 0
                    ? Math.round(((dashboardData.stats.total_invoices - dashboardData.stats.paid_invoices - dashboardData.stats.overdue_invoices) / dashboardData.stats.total_invoices) * 100)
                    : 0}% of total
                </p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="text-sm text-gray-500">
                  {dashboardData.stats.total_invoices > 0
                    ? Math.round((dashboardData.stats.overdue_invoices / dashboardData.stats.total_invoices) * 100)
                    : 0}% of total
                </p>
              </div>
            </div>
